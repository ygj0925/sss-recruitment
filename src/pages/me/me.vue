...
<template>
  <view class="app-tab-page font-['Noto_Sans_SC','Microsoft_YaHei',sans-serif]">
    <view class="box-border min-h-650rpx px-52rpx pb-156rpx text-white bg-brand-hero">
      <AppPageHeader
        title="我的"
        light
        transparent
      ...
      />
      <view
        class="mt-68rpx flex items-center"
        :class="!tokenStore.hasLogin ? 'cursor-pointer active:opacity-82' : ''"
        :role="tokenStore.hasLogin ? undefined : 'button'"
        @tap="handleProfileTap"
      >
        <view
          class="mr-24rpx h-112rpx w-112rpx flex flex-[0_0_112rpx] items-center justify-center border-2rpx border-[rgba(255,255,255,0.22)] rounded-full border-solid bg-[rgba(255,255,255,0.22)] text-44rpx text-white font-700 shadow-[inset_0_2rpx_8rpx_rgba(255,255,255,0.14)]"
        >
          <text v-if="tokenStore.hasLogin">{{ profileInitial }}</text>
          <view v-else class="i-carbon-user-avatar text-64rpx" />
        </view>
    ...
    <view class="relative z-1 px-32rpx -mt-78rpx">
      <view class="rounded-28rpx bg-white px-4rpx py-30rpx shadow-[0_16rpx_40rpx_rgba(42,92,178,0.12)]">
        <view class="grid grid-cols-4">
          <view
            v-for="shortcut in resumeShortcuts"
            :key="shortcut.label"
            class="min-h-158rpx min-w-0 flex flex-col cursor-pointer items-center justify-center text-27rpx text-app-ink leading-[1.4] active:bg-[#f7f9fd]"
            role="button"
            @tap="handleProfileTap"
          >
            <view class="mb-18rpx text-56rpx text-brand-blue" :class="[shortcut.icon]" />
      ...
      <view class="mt-34rpx">
        <text class="mx-8rpx mb-16rpx block text-32rpx text-app-ink font-700">求职服务</text>
        <view class="rounded-28rpx bg-white px-28rpx">
      ...
      <view v-if="!tokenStore.hasLogin" class="mt-24rpx flex flex-col items-center rounded-28rpx bg-white px-28rpx py-30rpx text-center">
        ...
        <navigator
          ...
          open-type="navigate"
          hover-class="none"
          class="mt-26rpx box-border min-h-84rpx w-full flex cursor-pointer items-center justify-center rounded-14rpx bg-brand-blue text-30rpx text-white font-600 transition-[opacity,transform] duration-180 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-99 active:opacity-88 motion-reduce:transition-none"
        >
          前往登录
        </navigator>
      </view>
      ...
      <view
        v-else
        class="mt-24rpx min-h-92rpx flex cursor-pointer items-center justify-center gap-12rpx rounded-28rpx bg-white text-29rpx text-[#df555b] font-600 active:bg-[#fff7f7]"
        role="button"
        @tap="handleLogout"
      >
